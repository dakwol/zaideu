import { CREATE_PROJECT_VALIDATION_MESSAGES } from '../model/constants'
import { CreateProjectStep, type CreateProjectFormValues } from '../model/types'
...
const getIdeaValidationMessages = (formValues: CreateProjectFormValues): string[] => {
...
const getStarterTasksValidationMessages = (formValues: CreateProjectFormValues): string[] => {
  const validationMessages: string[] = []
  if (formValues.tasks.length === 0) {
    validationMessages.push(CREATE_PROJECT_VALIDATION_MESSAGES.missingTasks)
  }
  const hasEmptyTaskTitle = formValues.tasks.some(projectTask => !projectTask.title.trim())
