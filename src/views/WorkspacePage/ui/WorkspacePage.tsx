...
function WorkspacePage() {
  ...
  const suggestedNextTasks = workspaceState.tasks
    .filter((projectTask) =>
      [
        ProjectTaskStatus.Available,
        ProjectTaskStatus.InProgress,
      ].includes(projectTask.status),
    )
    .slice(0, 3)
    .map((projectTask) => ({
      task: projectTask,
      project: mockProjects.find(
        (mockProject) => mockProject.id === projectTask.projectId,
      ),
    }))

  const projectsNeedHelp = mockProjects
    .filter(
      (project) => project.status === 'slow' || project.status === 'stalled',
    )
    .slice(0, 3)
    .map((project) => ({
      project,
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className={styles.main}>
        <header className={styles.header}>
          ...
          <p>
            Рабочая зона показывает ближайшее действие, движение команды и
            маленькие задачи, к которым можно подключиться.
          </p>
        </header>
