...
export const ProfilePage = () => {
  ...
  const userProjects = useMemo(() => {
    return mockProjects.filter(project =>
      project.participants.some(user => user.id === currentUser.id)
    )
  }, [])
  const userActivities = useMemo(() => {
    return mockActivities
      .filter(activity => activity.user.id === currentUser.id)
      .sort((activityA, activityB) => activityB.timestamp.getTime() - activityA.timestamp.getTime())
      .slice(0, 5)
  }, [])
  const stats = useMemo(() => {
    const allTasks = userProjects.flatMap(project => project.stages.flatMap(stage => stage.tasks))
    const myCompletedTasks = allTasks.filter(
      task => task.completed && task.assignedUser?.id === currentUser.id
    )
    ...
    return {
      ...
      activeProjects: userProjects.filter(
        project => project.status === 'active' || project.status === 'revival'
      ).length,
  ...
  const activityLevelConfig = {
    high: {
      label: translate('profile.activityLevelHigh'),
      className: 'bg-success/10 text-success',
      progress: 85,
    },
    medium: {
      label: translate('profile.activityLevelMedium'),
      className: 'bg-warning/10 text-warning',
      progress: 50,
    },
    low: {
      label: translate('profile.activityLevelLow'),
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
        <div className="flex flex-col sm:flex-row items-start gap-6 mb-8 pb-8 border-b border-border">
          ...
          <div className="flex-1">
            ...
            <p className="text-muted-foreground mb-4">
              {translate('profile.memberSince', {
                date: new Date(Date.now() - 86400000 * 90).toLocaleDateString('en-US', {
                  month: 'long',
                  year: 'numeric',
                }),
              })}
          ...
          <div className="flex flex-col items-end gap-2">
            <div className="flex items-center gap-2">
              <span className="text-sm text-muted-foreground">
                {translate('profile.activityLevel')}
              </span>
              <span
                className={`px-2 py-0.5 text-xs font-medium rounded ${activityConfig.className}`}
              >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
          <div className="p-4 bg-card border border-border rounded-lg">
            <div className="flex items-center gap-2 text-muted-foreground mb-2">
              ...
              <span className="text-xs font-medium uppercase tracking-wide">
                {translate('profile.completed')}
              </span>
            </div>
            <p className="text-2xl font-bold text-foreground">{stats.completedTasks}</p>
            <p className="text-xs text-muted-foreground">{translate('profile.tasksLabel')}</p>
          </div>
          ...
          <div className="p-4 bg-card border border-border rounded-lg">
            ...
            <p className="text-xs text-muted-foreground">{translate('profile.projectsLabel')}</p>
          </div>
        ...
        <div>
          <h2 className="text-lg font-semibold text-foreground mb-4">
            {translate('profile.recentActivity')}
          </h2>
          <div className="space-y-3">
            {userActivities.length > 0 ? (
              ...
              <p className="text-muted-foreground text-sm py-4">
                {translate('profile.noRecentActivity')}
              </p>
            )}
          </div>
        </div>
