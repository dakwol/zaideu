...
const ProjectContributors = ({ contributors, tasks }: ProjectContributorsProps) => {
  return (
    <section className={styles.section}>
      ...
      <div className={styles.list}>
        {contributors.map(contributor => {
          const currentTask = tasks.find(task => task.assigneeId === contributor.id)
          return (
            <article className={styles.card} key={contributor.id}>
              ...
              <div>
                ...
                {currentTask ? <strong>Сейчас работает над {currentTask.title}</strong> : null}
              </div>
            </article>
          )
        })}
