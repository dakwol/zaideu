...
import { TaskCard } from '@/entities/project/ui/TaskCard'
...
export const StageCard = ({ stage, isActive = false, className, onTakeTask }: StageCardProps) => {
    ...
    >
      ...
      >
        <div className="flex items-center justify-between gap-3 mb-2">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-semibold text-sm text-card-foreground">{stage.title}</h3>
          </div>
        ...
        <p className="text-xs text-muted-foreground mb-3">{stage.goal}</p>
      ...
      <div className="p-3 space-y-2">
        {stage.tasks.map(task => (
          <TaskCard key={task.id} task={task} onTakeTask={onTakeTask} />
        ))}
      </div>
    </div>
  )
