...
type ActivityFilter = 'all' | 'task_completed' | 'stage_completed' | 'user_joined' | 'user_update' | 'project_update'

const filterOptions: { value: ActivityFilter; labelKey: string }[] = [
...
export default function ActivityPage() {
  ...
  const allActivities = useMemo(() => {
    ...
    mockProjects.forEach(project => {
      project.stages.forEach(stage => {
        stage.tasks.filter(task => task.completed && task.assignedUser).forEach(task => {
          expanded.push({
            id: `gen-${task.id}`,
            type: 'task_completed',
            user: task.assignedUser!,
            description: t('activity.completedInProject', { task: task.title, project: project.title }),
    ...
    return expanded.sort((a, b) => b.timestamp.getTime() - a.timestamp.getTime())
  }, [t])
  ...
  const getProjectTitle = (projectId: string) => {
    return mockProjects.find(p => p.id === projectId)?.title || t('activity.unknownProject')
  }
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-foreground mb-2">{t('activity.title')}</h1>
